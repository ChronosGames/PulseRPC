...
public class HealthCheckServiceTests : IDisposable
{
    ...
    public HealthCheckServiceTests()
    {
        ...
        _healthCheckService = new HealthCheckService(optionsMock.Object, _mockLogger.Object);
    }
    ...
    public async Task MonitorHealthAsync_ShouldThrowArgumentNullException_WhenEndpointIsNull()
    {
        ...
        await Assert.ThrowsAsync<ArgumentNullException>(() =>
            _healthCheckService.MonitorHealthAsync(null!, TimeSpan.FromSeconds(1)).GetAsyncEnumerator().MoveNextAsync().AsTask());
    }
    ...
    public async Task MonitorHealthAsync_ShouldThrowArgumentException_WhenIntervalIsZero()
    {
        ...
        await Assert.ThrowsAsync<ArgumentException>(() =>
            _healthCheckService.MonitorHealthAsync(endpoint, TimeSpan.Zero).GetAsyncEnumerator().MoveNextAsync().AsTask());
    }
    ...
    public async Task MonitorHealthAsync_ShouldThrowArgumentException_WhenIntervalIsNegative()
    {
        ...
        // Act & Assert
        await Assert.ThrowsAsync<ArgumentException>(() =>
            _healthCheckService.MonitorHealthAsync(endpoint, TimeSpan.FromMilliseconds(-1)).GetAsyncEnumerator().MoveNextAsync().AsTask());
    }
    ...
    public async Task MonitorHealthAsync_ShouldYieldHealthCheckResults_WhenEndpointIsValid()
    {
        ...
        // Act
        await foreach (var result in _healthCheckService.MonitorHealthAsync(endpoint, interval, cts.Token))
        {
            results.Add(result);
