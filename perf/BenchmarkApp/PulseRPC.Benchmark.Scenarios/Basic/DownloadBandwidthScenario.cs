...
/// 测试服务端到客户端的数据传输带宽
/// </summary>
public class DownloadBandwidthScenario(ILoggerFactory loggerFactory) : BenchmarkClientBase(loggerFactory)
{
    ...
    public override string ScenarioName => "Download Bandwidth";
    public override string Description => "测试服务端到客户端的数据传输带宽";
    ...
    public override async Task<BenchmarkResult> ExecuteScenarioAsync(BenchmarkConfiguration config, CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            Logger.LogInformation("开始下行带宽测试，目标迭代次数: {Iterations}, 并发连接数: {Connections}, 数据包大小: {Size}B",
            ...
            Logger.LogInformation("下行带宽测试完成，总操作数: {Total}, 成功: {Success}, 带宽: {Bandwidth:F2} MB/s, 平均延迟: {Latency:F2}ms",
                _completedOperations + _failedOperations, _completedOperations, bandwidthMBps, result.Latency.AverageMs);
        }
        catch (Exception ex)
        {
            Logger.LogError(ex, "下行带宽测试失败");
    ...
    private async Task WarmupPhase(BenchmarkConfiguration config, CancellationToken cancellationToken)
    {
        ...
        for (int i = 0; i < warmupIterations; i++)
        {
            ...
            await ExecuteSingleDownloadOperation(payloadSize, i, cancellationToken);
        }
    ...
    private async Task ExecuteConnectionTestAsync(int connectionId, int iterations, int payloadSize, CancellationToken cancellationToken)
    {
        try
        {
            Logger.LogDebug("连接 {ConnectionId} 开始执行 {Iterations} 次下行操作", connectionId, iterations);
            ...
            for (int i = 0; i < iterations; i++)
            {
                ...
                try
                {
                    var (operationTime, receivedBytes) = await ExecuteSingleDownloadOperation(payloadSize, i, cancellationToken);

                    lock (_lock)
                    {
                        _operationTimes.Add(operationTime);
                        _completedOperations++;
                        _totalBytesReceived += receivedBytes;
                    }
                }
                catch (Exception ex)
                {
                    Logger.LogDebug("连接 {ConnectionId} 下行操作失败: {Error}", connectionId, ex.Message);
                    Interlocked.Increment(ref _failedOperations);
                }
            }

            Logger.LogDebug("连接 {ConnectionId} 完成所有下行操作", connectionId);
        }
        catch (Exception ex)
        {
    ...
    private async Task<(double operationTimeMs, int receivedBytes)> ExecuteSingleDownloadOperation(int payloadSize, int sequenceNumber, CancellationToken cancellationToken)
    {
        ...
        var request = DownloadRequest.Create(requestId, sequenceNumber, payloadSize);

        var response = await BenchmarkService.DownloadAsync(request, cancellationToken);

        stopwatch.Stop();

        if (!response.Success)
        {
            throw new InvalidOperationException($"下行请求失败: {response.ErrorMessage}");
        }

        return (stopwatch.Elapsed.TotalMilliseconds, response.Payload?.Length ?? 0);
    }
    ...
    private void PopulateStatistics(BenchmarkResult result, TimeSpan totalTime, int payloadSize)
    {
        ...
        result.Throughput.TotalBytesTransferred = _totalBytesReceived;
        ...
        if (totalTimeSeconds > 0)
        {
            ...
            result.Throughput.AverageBandwidthBps = _totalBytesReceived / totalTimeSeconds;
        }
