...
| P-2 | 稳定方法号（MethodId/protocolId）+ 冲突即报错（不导出清单文件） | `RouteByProtocolIdAsync(ushort)` 已按数值号分发；**协议号已由生成器确定性分配**（`ProtocolIdGenerator` 用 FNV-1a 哈希方法签名），但**冲突时用线性探测（+1）可能致号漂移**，且**无随代码提交的清单文件、无 CI 校验** | ~~①冲突稳定化；②导出清单文件；③CI 漂移校验~~ → **改为**：①去掉线性探测，协议号变为方法签名的纯哈希函数；②冲突（手动号之间/自动号之间/手动号与自动号之间）一律编译期报错，要求用 `[Protocol(0xXXXX)]` 显式区分；③客户端 `AssignProtocolIds` 由逐接口聚合改为整编译单元聚合，与服务端可见域对齐，消除跨端号不一致的隐藏 bug；④算法/签名格式变更视为 Major 版本破坏性变更（文档 + CHANGELOG 纪律兜底，不做清单+CI） | §4.5 信封寻址 | 高 |<br>**✅ 已实施**：`ProtocolIdHelper.GenerateProtocolId`/客户端 `ProtocolIdGenerator.GenerateProtocolId` 均改为纯哈希函数，移除 `GenerateUniqueProtocolId` 探测逻辑；服务端 `AssignProtocolIdsForIncremental`（Hub）与新增的 `AssignReceiverProtocolIds` 冲突分支（`PULSE003`/`PULSE004`）、客户端 `ReportProtocolIdConflict`（`PRPC001`）在冲突时报编译错误；客户端新增 `AssignProtocolIds(IEnumerable<INamedTypeSymbol>, context)` 重载，`ServiceProxyGenerator` 对 Hub 与 Receiver 分别一次性聚合编译单元内所有接口计算协议号，冲突可见域与服务端一致；`PulseRPC.Server`/`ProtocolAttribute` 文档同步更新；全解决方案 `dotnet build`/`dotnet test` 通过，无既有冲突。<br>**追加实施（2）**：①`IPulseReceiver` 方法补齐 `[Protocol(0xXXXX)]` 手动指定支持（此前仅 Hub 方法支持，Receiver 方法此前完全没有解决冲突的手段）——服务端 `CreateReceiverModelFromSymbol` 读取特性并注入 `ReceiverMethodModel.ProtocolId`，`AssignReceiverProtocolIds` 改为「手动优先、自动兜底」两阶段分配（与 Hub 侧一致）；客户端因 3.3 已将 Receiver 纳入统一聚合入口，`[Protocol]` 天然生效，无需改动。②新增 Roslyn `CodeFixProvider`（`ProtocolIdConflictCodeFixProvider`，服务端/客户端各一份，分别响应 `PULSE003`/`PULSE004` 与 `PRPC001`）：生成器在报冲突时于 `Diagnostic.Properties["SuggestedProtocolId"]` 附带一个当前未占用的建议协议号，IDE 中一键插入 `[Protocol(0xXXXX)]`；为此同时把诊断定位从 `Location.None` 改为冲突方法的真实源码位置（`MethodModel`/`ReceiverMethodModel` 新增 `Location` 字段），CodeFix 才能锚定要插入特性的方法。两个生成器项目新增 `Microsoft.CodeAnalysis.*.Workspaces` 依赖（`PrivateAssets=all`，不随包分发），因此产生 RS1038（生成器与 CodeFixProvider 同程序集）告警——评估后判定为可接受的实践性权衡并显式 `NoWarn`（详见各 csproj 内注释），未拆分独立 CodeFix 程序集/包以避免大幅增加打包与多项目引用的复杂度。全解决方案 `dotnet build`/`dotnet test` 复测通过。 |
...
| P-5 | 只读信封头的**地址中转** API | 目标实例 key 需进入处理管线才可知 | 暴露「只读头（Hub/Key/MethodId）→ 转发到目标节点/实例、不反序列化 body」的中转原语，供 Gateway 作纯中转（doc 09 §5.1） | §5 Gateway 纯中转 | 高 |<br>**✅ 已实施**：①信封头新增实例 Key——`MessageHeader.ServiceKey`（`[MemoryPackOrder(8)]`，追加尾部成员，前后向兼容：旧数据反序列化为空串、旧代码读新数据跳过多余成员），使「目标实例 key」随线格式头一并到达，无需进管线即可知。②新增只读头视图 `PulseRPC.Messaging.ReadOnlyEnvelopeHeader`（`readonly struct`，仅暴露 Type/MessageId/Hub/Key/MethodId/Flags，含 `FromHeader`/`ToHeader`/`WithKey`/`WithHub`）。③新增中转原语 `PulseRPC.Messaging.EnvelopeRelay`（static）：`TryReadHeader(ReadOnlySpan/ReadOnlyMemory<byte> frame, out ReadOnlyEnvelopeHeader header, out body)` 复用 `MessagePacket.TryReadFrom`——**只反序列化小头、body 全程保持 opaque**（已用随机非法 MemoryPack 字节作 body 验证仍可中转）；`GetFrameSize`/`TryWriteFrame`/`WriteFrame` 支持「改写头（如改 Key/Hub）+ body 原样拼接」重组帧，供 Gateway 纯中转到目标节点/实例。④管线/上下文贯通：`IPulseContext.ServiceKey`（默认接口实现 `=> string.Empty`，非破坏）、`PulseContextData.ServiceKey`（init）、`PulseContext.CurrentServiceKey` 便捷访问器，`HighPerformanceMessageEngine.ProcessSingleMessage` 从 `envelope.Header.ServiceKey` 注入上下文。⑤新增 17 个单元测试（EnvelopeRelay 11 + MessageHeader.ServiceKey 6）覆盖只读头解析、body opaque、改键重组、截断帧、MemoryPack 往返与跨版本兼容；`PublicAPI.Unshipped.txt` 已登记；全解决方案 `dotnet build`/`dotnet test` 通过。<br>范围说明：本项交付「中转原语 + Key 入头 + 管线/上下文贯通」；客户端存根填充 `ServiceKey`、服务端按 key 的 keyed 路由/归属属 P-1/P-7 关注点，不在本项内（`ServiceKey` 为 public get/set，任意层均可填充）。 |
...
| --- | --- | --- |
| PulseRPC | P-1 通用 `IPulseHub` 生成器；P-2 稳定方法号（冲突即报错，✅已实施）；P-4 反向 Ask；P-5 头部地址中转；P-6 可见性门闸（P-3 重入为增强） | P-7 单激活归属；P-8 持久化；P-9 reminders；P-10 Tick |
