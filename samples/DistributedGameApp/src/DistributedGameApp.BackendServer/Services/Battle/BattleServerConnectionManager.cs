...
/// <summary>
/// BattleServer 连接管理器 - 负责管理到多个 BattleServer 的连接池
/// </summary>
public class BattleServerConnectionManager : IAsyncDisposable
{
    private readonly ConsulServiceDiscovery _serviceDiscovery;
    private readonly ILoggerFactory _loggerFactory;
    private readonly ILogger<BattleServerConnectionManager> _logger;
    private readonly ConcurrentDictionary<string, BattleServerConnection> _connections = new();
    ...
    public BattleServerConnectionManager(
        ConsulServiceDiscovery serviceDiscovery,
        ILoggerFactory loggerFactory,
        string serviceType = "BattleServer",
    ...
    public async Task InitializeAsync(CancellationToken cancellationToken = default)
    {
        if (_isInitialized)
        {
            _logger.LogWarning("BattleServerConnectionManager 已经初始化");
            return;
        }
        ...
        try
        {
            ...
            _logger.LogInformation("初始化 BattleServerConnectionManager (类型: {ServiceType})...", _serviceType);
            ...
            _isInitialized = true;
            _logger.LogInformation("BattleServerConnectionManager 初始化完成,已连接 {Count} 个实例",
                _connections.Count);
        }
        finally
        {
    ...
    /// 获取指定服务的连接
    /// </summary>
    public BattleServerConnection? GetConnection(string serviceId)
    {
    ...
    private async Task AddConnectionAsync(ServiceRegistration service, CancellationToken cancellationToken)
    {
        ...
        _logger.LogInformation("添加 BattleServer 连接: {ServiceId} at {Host}:{Port}",
            service.ServiceId, service.Host, service.TcpPort);

        var connectionLogger = _loggerFactory.CreateLogger<BattleServerConnection>();
        var connection = new BattleServerConnection(service, connectionLogger, _authenticationProvider);
    ...
    private async Task RemoveConnectionAsync(string serviceId)
    {
        ...
        {
            _logger.LogInformation("移除 BattleServer 连接: {ServiceId}", serviceId);
    ...
    {
        if (sender is BattleServerConnection connection)
        {
    ...
    public async ValueTask DisposeAsync()
    {
        ...
        _logger.LogInformation("正在释放 BattleServerConnectionManager...");
        ...
        _logger.LogInformation("BattleServerConnectionManager 已释放");
    }
}
...
public class ConnectionChangeEvent
{
    ...
    public BattleServerConnection? Connection { get; }

    public ConnectionChangeEvent(
        ConnectionChangeType type,
        string serviceId,
        BattleServerConnection? connection)
    {
        Type = type;
        ServiceId = serviceId;
