...
/// <summary>
/// BackendServer 路由器 - 使用一致性哈希进行分片路由
/// </summary>
public class BackendServerRouter : IDisposable
{
    private readonly BackendServerConnectionManager _connectionManager;
    private readonly ILogger<BackendServerRouter> _logger;
    private readonly ConsistentHash<string> _consistentHash;
    private readonly Dictionary<string, BackendServerConnection> _serviceIdToConnection = new();
    ...
    public BackendServerRouter(
        BackendServerConnectionManager connectionManager,
        ILogger<BackendServerRouter> logger,
    ...
    /// 根据路由键获取连接（用于分片）
    /// </summary>
    /// <param name="routingKey">路由键（通常是 UserId 或 PlayerId）</param>
    public BackendServerConnection? GetConnection(string routingKey)
    {
    ...
    /// 根据路由键获取多个连接（用于容错/备份）
    /// </summary>
    public List<BackendServerConnection> GetConnections(string routingKey, int count)
    {
        if (string.IsNullOrEmpty(routingKey))
            throw new ArgumentException("Routing key cannot be null or empty", nameof(routingKey));
        ...
        lock (_lock)
        {
            ...
            var connections = new List<BackendServerConnection>();
    ...
    /// 使用一致性哈希获取连接
    /// </summary>
    private BackendServerConnection? GetConnectionByConsistentHash(string routingKey)
    {
    ...
    /// 使用轮询获取连接
    /// </summary>
    private BackendServerConnection? GetConnectionByRoundRobin()
    {
    ...
    /// 使用随机获取连接
    /// </summary>
    private BackendServerConnection? GetConnectionByRandom()
    {
        var connections = _serviceIdToConnection.Values
            .Where(c => c.State == ConnectionState.Connected)
            .ToList();
    ...
    /// 使用最少负载获取连接
    /// </summary>
    private BackendServerConnection? GetConnectionByLeastLoad()
    {
    ...
    /// 将节点添加到哈希环
    /// </summary>
    private void AddNodeToHashRing(BackendServerConnection connection)
    {
    ...
    public void Dispose()
    {
        ...
        _logger.LogInformation("BackendServerRouter 已释放");
    }
}
