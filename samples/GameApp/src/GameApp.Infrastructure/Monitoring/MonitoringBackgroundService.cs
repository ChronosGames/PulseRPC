    ...
    : BackgroundService
{
    ...
    {
        logger.LogInformation("监控告警后台服务已启动");
        ...
        while (!stoppingToken.IsCancellationRequested)
        {
            ...
            {
                logger.LogError(ex, "监控告警后台服务执行出错");
        ...
        logger.LogInformation("监控告警后台服务已停止");
    }
    ...
    private async Task CheckAlertRulesAsync(IServiceProvider services)
    {
        try
        {
            ...
            logger.LogDebug("告警规则检查完成");
        }
        catch (Exception ex)
        {
            logger.LogError(ex, "检查告警规则失败");
    ...
    private async Task PerformHealthCheckAsync(IServiceProvider services)
    {
        try
        {
            ...
            logger.LogDebug("系统健康检查完成，健康分数: {HealthScore}%", healthScore);
        }
        catch (Exception ex)
        {
    ...
    private async Task PerformMaintenanceAsync(IServiceProvider services)
    {
        try
        {
            logger.LogInformation("开始执行维护任务...");
            ...
            logger.LogInformation("维护任务执行完成");
        }
        catch (Exception ex)
        {
            logger.LogError(ex, "执行维护任务失败");
        }
    }
    ...
    private async Task CleanupResolvedAlertsAsync(IAlertService alertService)
    {
        try
        {
            ...
            logger.LogDebug("告警清理任务完成");
        }
        catch (Exception ex)
        {
            logger.LogError(ex, "清理已解决告警失败");
        }
    }
    ...
    private async Task CheckLongRunningAlertsAsync(IAlertService alertService, IStructuredLogger structuredLogger)
    {
        ...
        {
            logger.LogError(ex, "检查长时间运行告警失败");
        }
    }
    ...
    private async Task GenerateMaintenanceReportAsync(IServiceProvider services, IStructuredLogger structuredLogger)
    {
        ...
        {
            logger.LogError(ex, "生成维护报告失败");
        }
    }

    public override async Task StopAsync(CancellationToken cancellationToken)
    {
        logger.LogInformation("正在停止监控告警后台服务...");
