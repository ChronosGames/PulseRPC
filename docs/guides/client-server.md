...
客户端接收服务端推送时，接收契约标注 `[Channel("CLIENT")]`；旧 `IPulseReceiver` 已从公共 API 硬移除：
...
`MessageWorkerShardCount` 默认等于逻辑处理器数，控制固定的消息 worker 数和最大在途分发并发；`MessageQueueCapacityPerShard` 默认是 `1024`。连接注册后会在其生命周期内固定绑定一个 shard。队列满时新消息立即被拒绝，不会通过同步等待或 adaptive batching 隐式吸收压力。生产调优前先观察 `RuntimeQueueMetrics` 的深度、饱和和拒绝计数，不要使用已废弃的 L1/L2/L3、`MessageEngineConfiguration` 或 `ServerPreset` 配置。
...
旧 `IPulseReceiver` 已硬移除，不存在可继续运行的兼容接口。保留原 Receiver 的接口名和方法，只改为当前契约声明：
...
仓库中的迁移分析器和 CodeFix 会识别旧继承语法，完成 `[Channel("CLIENT")] : IPulseHub` 的机械迁移；随后应重新生成客户端 Dispatcher 与服务端推送代理。
