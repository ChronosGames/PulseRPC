...
本地或远程 Actor 只有在 `OnStartingAsync` 成功、实例正式发布后才进入续租心跳。启动失败会回滚未发布实例并 compare-release 当前租约；实例驱逐或服务管理器关闭时也会先停止跟踪，再释放同一 lease id，避免已回收 Actor 继续形成幽灵租约。

同一服务地址的创建、正在创建中的发布、停止、Dispose 和租约释放由 `PulseServiceManager` 串行化。移除请求会等待共享激活任务完成，新一轮创建则必须等旧实例完成 Dispose 和 lease release，避免移除错过尚未发布实例或新旧 owner 短暂并存。

L3 迁移对同一 `(Hub, Key)` 串行执行。迁出顺序为静默排空、捕获/传输快照、Dispose 本地实例、停止心跳、释放 fencing lease；迁入仅在恢复与启动全部成功后跟踪心跳，失败会 Dispose 未发布实例并补偿释放刚取得的租约。
