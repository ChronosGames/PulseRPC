...
- [x] Service 生命周期：`PulseServiceManager` / `IServiceAccessor<T>` 是唯一推荐模型，注册目录在 Manager 首次解析时生效，不依赖 Host 启动顺序；旧 Service/Hub Factory 仅保留兼容测试并已标记 `[Obsolete]`。
...
- [x] `ServiceFactoryExample` 与历史命名的 `HubFactoryExample` 均已重写为当前 `AddPulseService` / `IServiceAccessor` API；`JsonTranscoding` 已改为可构建的显式 ASP.NET JSON 网关样例。
...
- [x] 延迟统计使用固定内存、并发累计且可合并的 histogram，不再只保留最近 100–1024 个样本；消息引擎收敛为固定 worker shard + 每 shard 有界队列，不再构造每连接分层处理器、自适应调度或 L3 内存池。Engine 与 Pipeline 使用真实分桶语义，Prometheus 导出标准 bucket/sum/count。所有实际有界运行时队列注册真实 capacity/depth，并提供 saturation、高水位、饱和事件、入队等待和拒绝计数；诊断端点不再返回伪造的 L1/L2/L3 容量。
