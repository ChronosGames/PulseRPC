...
- **P1-8 服务端收包去 `MessagePacketHolder.Payload.ToArray()`**（已完成，含单元 + 并发压力测试验证）：
  - `MessagePacketHolder` 改为持有 `System.Buffers.MemoryPool<byte>` 池化缓冲（`IMemoryOwner<byte>`）：构造时把载荷从收包缓冲**复制进池化缓冲**（取代每消息堆分配的 `ToArray`），并实现 `IDisposable`；`Dispose` 用 `Interlocked` 守卫做到**幂等**（重复归还安全、不会二次归还池）。
  - 所有权沿管道流转：`MessageSlot.PayloadOwner` 承载该 owner；确定性归还落在**每条终结分支**——`MessageEngine.TryEnqueueMessage`（processor 未找到 / 异常）、`HandleBackpressure`（Normal 高负载丢弃 / Low 丢弃 / default）、`TryEnqueueWithRetry`（重试耗尽）各分支就地归还；入队成功后所有权移交 processor，`TieredMessageProcessor.ProcessMessageSlot` 以**最外层 `finally`** 覆盖 Deadline 卸载与正常处理两条出口归还；关闭时 L1 排空处归还。`ServerTransportChannel` 在无 `MessageParsed` 订阅者时就地归还。
  - 载荷新契约：`Payload` 生命周期限于 handler 调用内（handler 在调用期间同步反序列化消费，`await` 返回后不得再引用）；已核实现有链路 `handler → envelope.Payload → await ProcessSingleMessage` 满足该契约。
  - **过程中发现并修复两个既有严重 bug**（非 P1-8 引入，但直接威胁其正确性/可测性）：
    1. `ZeroCopyCircularBuffer.PrewarmBuffer` 步长 `64 / Unsafe.SizeOf<T>()` 为**整数除法**：当元素结构体 ≥ 一个缓存行（64B）时步长=0 → `i += 0` **构造函数死循环**。`MessageSlot` 因新增 `PayloadOwner` 引用字段跨过 64B 而触发（生产环境每连接构造一次 `TieredMessageProcessor` 即卡死）。已钳步长下限为 1。
    2. `TieredMessageProcessor.L1ToL2BatchTransferLoop` 的 `batchBuffer` 在 `while` 外**只分配一次、跨批次复用**，而每批 `Messages` 是指向该共享数组的 `ReadOnlyMemory<MessageSlot>`：通道可缓冲多批时，后续迭代会覆写仍在处理的批次内存 → 撕裂含引用的 `MessageSlot`（→ 载荷二次归还/泄漏、乃至读到非法 offset/length 崩溃）。已改为**每批分配独立数组**（并清理临时缓冲引用）。P1-8 的幂等 `Dispose` 守卫本身也能挡住该类二次归还导致的池损坏。
  - 测试（`tests/PulseRPC.Server.Tests/Processing/`）：`MessagePacketHolderTests`（10 项：拷贝隔离/精确长度/空载荷/幂等 + 并发恰好一次归还）；`TieredMessageProcessorLifecycleTests`（处理完成、背压接受/拒收各分支断言「恰好归还一次」+ 载荷内容完好）。整套 `PulseRPC.Server.Tests` 48 通过 / 0 失败，进程可正常快速退出。
  - **跟进（代码整洁化，无行为变更）**：`MessageEngine.cs` 中原本散落的 6 处「拒绝入队 → 归还载荷池化缓冲 → return false」样板（`TryEnqueueMessage` 的 processor-未找到/异常兜底 2 处、`HandleBackpressure` 的 Normal/Low/default 丢弃 3 处、`TryEnqueueWithRetry` 重试耗尽 1 处）收敛为统一的私有辅助方法 `RejectSlot(IDisposable? payloadOwner)`，把「拒收即归还」表达为单一语义单元并集中文档说明；各调用点保留各自原有的日志/指标记录不变。整解决方案编译 0 错误，既有 P1-8 测试与 `PulseRPC.Server.Tests` 全量套件（48 通过 / 0 失败）重跑无回归。
- **P1-10 传输实现从 Abstractions 下沉到实现层**（已完成，整解决方案编译 0 错误验证；**破坏性打包变更**，发布时需版本协调）：
  - 新建 `PulseRPC.Shared` 项目（多目标 `netstandard2.1;net10.0`，`ProjectReference` → `PulseRPC.Abstractions`，单向依赖），加入 `PulseRPC.sln`。
  ...
  - `Client`/`Server` 各加一条 `ProjectReference` → `PulseRPC.Shared`；`TcpClientTransport`/`TcpServerTransport`/`Kcp*` 仍是继承同一份共享基类的**薄子类**（无重复实现）。
  ...
  - 打包影响：`TcpTransport` 等已从 `PulseRPC.Abstractions` 包移入新的 `PulseRPC.Shared` 包。仅引用 Abstractions 包并直接使用这些实现的下游需增加对 `PulseRPC.Shared` 的引用——发布时需相应的版本号/迁移说明。
