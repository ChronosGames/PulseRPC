...
1. IPulseClient.GetServiceAsync<T>()
        ↓
2. IConnectionRouter.RouteAsync() → Select Best IConnection
        ↓
3. IConnection.GetServiceAsync<T>() [Generated Proxy Creation]
        ↓
4. Source Generated ServiceProxy Constructor
        ↓
5. ServiceProxy.MethodAsync() [User calls specific method]
        ↓
6. IConnection.SendAsync<T>(RpcRequest) [Generated proxy calls]
        ↓
7. SimpleConnection.SendAsync<T>() [Delegation wrapper]
        ↓
8. IConnectionContext.InvokeAsync<T>() [Session layer processing]
        ↓
...
**步骤 1-2：应用层路由**
- `IPulseClient` 通过 `IConnectionRouter` 选择最佳连接
- 基于负载均衡策略、路由规则和连接健康状态

**步骤 3-4：服务代理创建**
- `IConnection.GetServiceAsync<T>()` 检查代理缓存
- 如果未缓存，通过 Source Generator 生成的工厂方法创建代理
- 代理构造函数接收 `IConnection` 实例

**步骤 5-6：方法调用和请求构造**
- 用户调用代理的具体方法（如 `userService.GetUserAsync(123)`）
- 生成的代理方法构造 `RpcRequest` 并调用 `IConnection.SendAsync<T>()`

**步骤 7-8：会话层处理**
- `SimpleConnection` 委托到 `IConnectionContext.InvokeAsync<T>()`
- 序列化请求、管理请求-响应映射、更新连接活动状态

**步骤 9-10：传输层发送**
- `IConnectionContext` 通过 `IClientTransport.SendAsync()` 发送数据
...
1. IClientTransport.DataReceived Event
        ↓
2. IConnectionContext.OnDataReceived() [Response matching]
        ↓
3. TaskCompletionSource<T>.SetResult() [Complete pending request]
        ↓
4. IConnectionContext.InvokeAsync<T>() returns result
        ↓
...
public sealed class ICalculatorServiceProxy : ICalculatorService
{
    ...
    public ICalculatorServiceProxy(IConnection connection)
    {
...
{
    public static T CreateServiceProxy<T>(this IConnection connection) where T : class, IPulseHub
    {
        if (connection == null)
...
//    - IConnectionRouter.RouteAsync() → selects best IConnection
//    - IConnection.GetServiceAsync<ICalculatorService>()
...
//    - SimpleConnection.SendAsync<int>() → IConnectionContext.InvokeAsync<int>()
