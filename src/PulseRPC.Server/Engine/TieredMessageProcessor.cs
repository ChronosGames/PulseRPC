...
public sealed class TieredMessageProcessor : IAsyncDisposable
{
    ...
    // 消息流水线
    private readonly Channel<MessageBatch> _batchChannel;
    private readonly ChannelReader<MessageBatch> _batchReader;
    private readonly ChannelWriter<MessageBatch> _batchWriter;
        ...
        ILogger<TieredMessageProcessor> logger)
    {
        ...
        // 初始化L2自适应调度器
        _l2Scheduler = new AdaptiveBatchScheduler(logger);
        ...
        var channel = Channel.CreateBounded<MessageBatch>(channelOptions);
    ...
    public bool TryEnqueueMessage(ReadOnlyMemory<byte> messageData, MessagePriority priority = MessagePriority.Normal)
    {
        ...
        {
            _metrics.L1MessagesEnqueued.Increment();
    ...
    {
        // 从L3内存池租用缓冲区
        var buffer = _l3MemoryPool.Rent(messageData.Length);
        messageData.CopyTo(buffer.Memory);
    ...
    {
        _metrics.L1BackpressureEvents.Increment();
        ...
        // 释放租用的内存
        _l3MemoryPool.Return(slot.Data);
        _metrics.MessagesDropped.Increment();
        return false;
    }
    ...
    private bool TryForceEnqueue(MessageSlot slot, TimeSpan timeout)
    {
        ...
        while (!cts.Token.IsCancellationRequested)
        {
            ...
            {
                _metrics.L1MessagesEnqueued.Increment();
                _metrics.ForcedEnqueues.Increment();
                return true;
            }
        ...
        while (!_cancellationTokenSource.Token.IsCancellationRequested)
        {
            try
            {
                       ...
                       _l1Buffer.TryDequeue(out var slot))
                {
                    ...
                    _metrics.L1MessagesDequeued.Increment();
                }
                ...
                if (batchSize > 0)
                {
                    ...
                    _metrics.BatchesCreated.Increment();
    ...
    private async Task BatchProcessingLoop()
    {
        await foreach (var batch in _batchReader.ReadAllAsync(_cancellationTokenSource.Token))
        {
            ...
            catch (Exception ex)
            {
                ...
                _metrics.BatchesErrored.Increment();
            }
        }
    }
    ...
    /// 处理消息批次
    /// </summary>
    private async Task ProcessBatch(MessageBatch batch)
    {
        var batchStartTime = Stopwatch.GetTimestamp();
    ...
    public ProcessorStatus GetStatus()
    {
        ...
        var l2Metrics = _options.EnableAdaptiveBatching ? _l2Scheduler.GetMetrics() : null;
