...
public sealed class LeaseActorDirectoryOptions
{
    ...
    /// <summary>
    /// 是否允许多节点拓扑继续使用 <see cref="InMemoryActorLeaseStore"/>。默认 <c>false</c>。
    /// </summary>
    /// <remarks>
    /// 仅供本地开发和同进程测试使用。生产多节点必须注册提供原子 CAS + TTL 的共享后端，
    /// 例如 <c>AddRedisActorLeases</c>；否则各进程拥有彼此独立的租约表，无法保证单一激活。
    /// </remarks>
    public bool AllowInMemoryStoreForMultiNode { get; set; }
}
