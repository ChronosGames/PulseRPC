    ...
    public async Task<bool> LockResource(string resourceKey, string requestId, TimeSpan timeout) {
        ...
        try {
            ...
            if (acquired) {
                ...
                _logger.LogDebug($"Local lock acquired: {resourceKey} by {requestId}");
                return true;
            }

            _logger.LogWarning($"Failed to acquire local lock: {resourceKey} by {requestId}, timeout");
            return false;
        }
        catch (Exception ex) {
            _logger.LogError($"Error acquiring local lock {resourceKey}: {ex.Message}");
            return false;
        }
    }
        ...
        if (_lockOwners.TryGetValue(resourceKey, out var currentOwner) &&
            ...
            if (_resourceLocks.TryGetValue(resourceKey, out var semaphore)) {
                try {
                    ...
                    _logger.LogDebug($"Local lock released: {resourceKey} by {requestId}");
                }
                catch (SemaphoreFullException) {
                    _logger.LogError($"Attempted to release unheld lock: {resourceKey}");
                }
            }
        }
        else {
            _logger.LogWarning($"Attempted to release lock {resourceKey} by non-owner {requestId}");
        }
