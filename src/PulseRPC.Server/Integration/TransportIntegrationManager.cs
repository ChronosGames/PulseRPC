...
internal sealed class TransportIntegrationManager : ITransportIntegrationManager
{
        ...
        ILogger<TransportIntegrationManager> logger)
    {
        ...
        _logger.LogInformation("传输层集成管理器已初始化，支持的传输类型: [{TransportTypes}]", string.Join(", ", _providers.Keys));
    }
    ...
    public IServerListener CreateListener(TransportChannelConfiguration config, ILoggerFactory loggerFactory)
    {
        ...
        {
            throw new NotSupportedException($"不支持的传输类型: {transportType}. 支持的类型: [{string.Join(", ", _providers.Keys)}]");
        }

        try
        {
            _logger.LogDebug("正在创建传输监听器: {TransportType}, 端口: {Port}, 名称: {Name}", transportType, config.Port, config.Name);

            var listener = provider.CreateServerListener(config, loggerFactory);

            _logger.LogInformation("已创建传输监听器: {TransportType}, 端口: {Port}, 名称: {Name}", transportType, config.Port, config.Name);
