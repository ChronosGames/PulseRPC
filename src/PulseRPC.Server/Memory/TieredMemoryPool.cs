...
public sealed class TieredMemoryPool : IDisposable
{
    ...
    public static TieredMemoryPool Instance
    {
        get
        {
            if (_instance == null)
            {
                lock (_instanceLock)
                {
                    _instance = _instance ?? new TieredMemoryPool();
                }
            }
            return _instance;
        }
    ...
    private bool TryGetFromThreadLocal(int minimumLength, out byte[] buffer)
    {
        _threadLocalCache = _threadLocalCache ?? new ThreadLocalBufferCache();
    ...
    /// 尝试从NUMA池获取缓冲区
    /// </summary>
    private bool TryGetFromNumaPool(int minimumLength, out byte[] buffer)
    {
    ...
    /// 尝试从全局池获取大缓冲区
    /// </summary>
    private bool TryGetFromGlobalPool(int minimumLength, out byte[] buffer)
    {
        var actualSize = CalculateBufferSize(minimumLength);
...
internal sealed class NumaAwareBufferPool : IDisposable
{
    ...
    public bool TryRent(int poolIndex, out byte[] buffer)
    {
