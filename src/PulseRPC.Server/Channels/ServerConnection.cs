...
public sealed class ServerConnection
{
    ...
    /// Connection state.
    /// </summary>
    public ServerConnectionState State { get; private set; }
    ...
    public bool TryTransitionState(ServerConnectionState newState)
    {
        // Validate state transition (one-way only)
        var isValidTransition = (State, newState) switch
        {
            (ServerConnectionState.Connecting, ServerConnectionState.Active) => true,
            (ServerConnectionState.Active, ServerConnectionState.Closing) => true,
            (ServerConnectionState.Closing, ServerConnectionState.Closed) => true,
            (ServerConnectionState.Connecting, ServerConnectionState.Closed) => true, // Failed connection
            _ => false
        };
    ...
    /// </summary>
    public bool IsActive => State == ServerConnectionState.Active;
