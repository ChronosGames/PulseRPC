...
public static class MessageDispatcherGenerator
{
    ...
    public static string GenerateSourceText(List<ServiceModel> serviceModels)
    {
        ...
        // 生成高性能路由方法（委托给 ServiceRoutingTable）
        GenerateRouteAsyncMethod(code, serviceModels);
    ...
    /// 生成 DI 扩展方法源代码（单独的文件）
    /// </summary>
    public static string GenerateDIExtensions(List<ServiceModel> serviceModels)
    {
        var code = new StringBuilder();
        ...
        // 生成 DI 扩展方法（单独的类）
        GenerateDIExtensionClass(code, serviceModels);
    ...
    private static void GenerateServiceInitializationMethod(StringBuilder code, List<ServiceModel> serviceModels)
    {
        ...
        code.AppendLine("        ");
        code.AppendLine("        _serviceProvider = serviceProvider;");
        code.AppendLine("        ");
        ...
        code.AppendLine("        ");
        code.AppendLine("        ");
        code.AppendLine("        // 初始化 ServiceRoutingTable 的 ServiceProvider");
        code.AppendLine("        PulseRPC.Generated.ServiceRoutingTable.ServiceProvider = serviceProvider;");
        code.AppendLine("        ");
    ...
    /// <summary>
    /// 生成 RouteAsync 方法 - 委托给 ServiceRoutingTable.RouteAsync
    /// </summary>
    private static void GenerateRouteAsyncMethod(StringBuilder code, List<ServiceModel> serviceModels)
    {
        code.AppendLine("    /// <summary>");
        code.AppendLine("    /// 高性能路由分发方法 - 委托给编译时生成的 ServiceRoutingTable");
        code.AppendLine("    /// 实现 AbstractCompiledMessageDispatcher.RouteAsync");
        code.AppendLine("    /// </summary>");
        code.AppendLine("    [MethodImpl(MethodImplOptions.AggressiveInlining)]");
        code.AppendLine("    public override ValueTask<object?> RouteAsync(");
        code.AppendLine("        string serviceName,");
        code.AppendLine("        string methodName,");
        code.AppendLine("        ReadOnlyMemory<byte> payloadData,");
        code.AppendLine("        CancellationToken cancellationToken = default)");
        ...
        code.AppendLine("        {");
        code.AppendLine("            throw new InvalidOperationException(\"CompiledMessageDispatcher not initialized. Call InitializeServices first.\");");
        code.AppendLine("        }");
        code.AppendLine("        ");
        code.AppendLine("        if (_serviceProvider == null)");
        code.AppendLine("        {");
        code.AppendLine("            throw new InvalidOperationException(\"ServiceProvider not set. Call InitializeServices first.\");");
        code.AppendLine("        }");
        code.AppendLine("        ");
        code.AppendLine("        // 直接委托给编译时生成的高性能路由表单例实例");
        code.AppendLine("        return PulseRPC.Generated.ServiceRoutingTable.Instance.RouteAsync(_serviceProvider, serviceName, methodName, payloadData, cancellationToken);");
        code.AppendLine("    }");
        code.AppendLine();
    }
    ...
    private static void GenerateStatisticsMethod(StringBuilder code, List<ServiceModel> serviceModels)
    {
        ...
        code.AppendLine("    private IServiceProvider? _serviceProvider;");
        code.AppendLine();
    ...
    /// 生成 DI 扩展类 - 提供自动注册功能
    /// </summary>
    private static void GenerateDIExtensionClass(StringBuilder code, List<ServiceModel> serviceModels)
    {
        code.AppendLine("namespace Microsoft.Extensions.DependencyInjection;");
        code.AppendLine();
        code.AppendLine("/// <summary>");
        code.AppendLine("/// PulseRPC 自动生成的 DI 扩展方法");
        code.AppendLine("/// 提供一键注册所有生成的服务和分发器");
        code.AppendLine("/// </summary>");
        code.AppendLine("public static class PulseRPCGeneratedServiceCollectionExtensions");
        code.AppendLine("{");
        code.AppendLine("    /// <summary>");
        code.AppendLine("    /// 注册 PulseRPC 生成的所有服务、分发器和路由表");
        code.AppendLine("    /// </summary>");
        code.AppendLine("    /// <param name=\"services\">服务集合</param>");
        code.AppendLine("    /// <returns>服务集合（支持链式调用）</returns>");
        code.AppendLine("    public static IServiceCollection AddPulseRPCGenerated(this IServiceCollection services)");
        code.AppendLine("    {");
        code.AppendLine("        if (services == null)");
        code.AppendLine("            throw new ArgumentNullException(nameof(services));");
        code.AppendLine();
        code.AppendLine("        // 注册编译时消息分发器");
        code.AppendLine("        services.AddSingleton<PulseRPC.Server.Engine.AbstractCompiledMessageDispatcher>(sp =>");
        code.AppendLine("        {");
        code.AppendLine("            var dispatcher = new PulseRPC.Generated.CompiledMessageDispatcher();");
        code.AppendLine("            dispatcher.InitializeServices(sp);");
        code.AppendLine("            return dispatcher;");
        code.AppendLine("        });");
        code.AppendLine();
        code.AppendLine("        // 注册 CompiledMessageDispatcher 为 IMessageDispatcher (如果需要)");
        code.AppendLine("        // 注意: 实际的 IMessageDispatcher 实现应该由应用程序自己注册");
        code.AppendLine();
        code.AppendLine("        return services;");
        code.AppendLine("    }");
        code.AppendLine();
        code.AppendLine("    /// <summary>");
        code.AppendLine("    /// 注册 PulseRPC 生成的所有服务、分发器和路由表（带配置选项）");
        code.AppendLine("    /// </summary>");
        code.AppendLine("    /// <param name=\"services\">服务集合</param>");
        code.AppendLine("    /// <param name=\"configureOptions\">配置分发器选项</param>");
        code.AppendLine("    /// <returns>服务集合（支持链式调用）</returns>");
        code.AppendLine("    public static IServiceCollection AddPulseRPCGenerated(");
        code.AppendLine("        this IServiceCollection services,");
        code.AppendLine("        Action<PulseRPC.Server.Dispatch.DispatcherOptions>? configureOptions)");
        code.AppendLine("    {");
        code.AppendLine("        if (services == null)");
        code.AppendLine("            throw new ArgumentNullException(nameof(services));");
        code.AppendLine();
        code.AppendLine("        var options = new PulseRPC.Server.Dispatch.DispatcherOptions();");
        code.AppendLine("        configureOptions?.Invoke(options);");
        code.AppendLine();
        code.AppendLine("        // 注册编译时消息分发器");
        code.AppendLine("        services.AddSingleton<PulseRPC.Server.Engine.AbstractCompiledMessageDispatcher>(sp =>");
        code.AppendLine("        {");
        ...
        code.AppendLine("        // 注册 CompiledMessageDispatcher 为 IMessageDispatcher (如果需要)");
        code.AppendLine("        // 注意: 实际的 IMessageDispatcher 实现应该由应用程序自己注册");
        code.AppendLine("        // options 参数已被忽略");
        code.AppendLine();
        code.AppendLine("        return services;");
        code.AppendLine("    }");
        code.AppendLine("}");
