...
/// <summary>
/// 针对 <see cref="ReceiverMigrationAnalyzer.DiagnosticId"/>（<c>PULSE_MIGRATE_RECEIVER</c>）诊断的自动修复。
/// </summary>
/// <remarks>
/// 把 <c>IXxxReceiver : IPulseReceiver</c> 机械改写为 <c>[Channel("CLIENT")] IXxxReceiver : IPulseHub</c>，
/// 对应《统一 IPulseHub 全链路寻址与集群架构设计》§13.2 迁移步骤第 1 步。方法签名与接口名不变，
/// 仅重写基类型与新增标记特性；协议号由生成器按新规则（§11.2）重新计算。
/// </remarks>
