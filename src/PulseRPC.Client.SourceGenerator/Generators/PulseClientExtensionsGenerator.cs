...
public static class PulseClientExtensionsGenerator
{
        ...
        ImmutableArray<INamedTypeSymbol> eventTypes)
    {
        ...
        // 生成自动注册方法
        GenerateAutoRegisterMethods(sb, eventTypes);
    ...
    private static void GenerateAutoRegisterMethods(StringBuilder sb, ImmutableArray<INamedTypeSymbol> eventTypes)
    {
        sb.AppendLine("        #region Auto-Register Methods");
        sb.AppendLine();

        // 按 ChannelName 分组事件类型
        var eventsByChannel = new Dictionary<string, List<INamedTypeSymbol>>();
        foreach (var eventType in eventTypes)
        {
            if (eventType == null) continue;

            var channelName = GetChannelName(eventType);
            if (!eventsByChannel.ContainsKey(channelName))
            {
                eventsByChannel[channelName] = new List<INamedTypeSymbol>();
            }
            eventsByChannel[channelName].Add(eventType);
        }

        // 生成主自动注册方法
        sb.AppendLine("        /// <summary>");
        sb.AppendLine("        /// 自动注册所有匹配指定 Channel 的事件监听器");
        sb.AppendLine("        /// </summary>");
        sb.AppendLine("        /// <param name=\"client\">PulseRPC 客户端实例</param>");
        sb.AppendLine("        /// <param name=\"channelName\">要匹配的 Channel 名称（如 \\\"CLIENT\\\"）</param>");
        sb.AppendLine("        /// <param name=\"handlers\">事件处理器实例数组</param>");
        sb.AppendLine("        /// <param name=\"options\">事件监听器选项</param>");
        sb.AppendLine("        /// <param name=\"cancellationToken\">取消令牌</param>");
        sb.AppendLine("        /// <returns>所有已注册接口的订阅令牌列表</returns>");
        sb.AppendLine("        public static async Task<IList<ISubscriptionToken>> AutoRegisterEventListenersAsync(");
        sb.AppendLine("            this IPulseClient client,");
        sb.AppendLine("            string channelName,");
        sb.AppendLine("            object[] handlers,");
        sb.AppendLine("            EventListenerOptions? options = null,");
        sb.AppendLine("            CancellationToken cancellationToken = default)");
        sb.AppendLine("        {");
        sb.AppendLine("            if (client == null) throw new ArgumentNullException(nameof(client));");
        sb.AppendLine("            if (string.IsNullOrWhiteSpace(channelName)) throw new ArgumentNullException(nameof(channelName));");
        sb.AppendLine("            if (handlers == null || handlers.Length == 0) throw new ArgumentException(\"至少需要提供一个事件处理器\", nameof(handlers));");
        sb.AppendLine();
        sb.AppendLine("            var tokens = new List<ISubscriptionToken>();");
        sb.AppendLine();
        ...
        // 为每个 Channel 生成匹配逻辑
        bool first = true;
        foreach (var kvp in eventsByChannel)
        {
            var channel = kvp.Key;
            var types = kvp.Value;
            ...
            sb.AppendLine($"            {condition} (string.Equals(channelName, \"{channel}\", StringComparison.OrdinalIgnoreCase))");
            sb.AppendLine("            {");
            sb.AppendLine($"                // 注册所有 {channel} Channel 的事件监听器");
            sb.AppendLine("                foreach (var handler in handlers)");
            sb.AppendLine("                {");
            sb.AppendLine("                    if (handler == null) continue;");
            sb.AppendLine();
            ...
            foreach (var eventType in types)
            {
                var interfaceName = eventType.Name;
                var fullTypeName = GetFullTypeName(eventType);
                if (string.IsNullOrEmpty(interfaceName) || string.IsNullOrEmpty(fullTypeName)) continue;

                sb.AppendLine($"                    // 检查是否实现了 {interfaceName}");
                sb.AppendLine($"                    if (handler is {fullTypeName} {interfaceName.ToLower()}Listener)");
                sb.AppendLine("                    {");
                sb.AppendLine($"                        try");
                sb.AppendLine($"                        {{");
                sb.AppendLine($"                            var connectionContext = await client.Connections.RouteAsync(\"{interfaceName}\", cancellationToken);");
                ...
                sb.AppendLine($"                        catch (Exception ex)");
                sb.AppendLine($"                        {{");
                sb.AppendLine($"                            // 记录错误但继续注册其他接口");
                sb.AppendLine($"                            System.Diagnostics.Debug.WriteLine($\"注册 {interfaceName} 失败: {{ex.Message}}\");");
                sb.AppendLine($"                        }}");
                sb.AppendLine("                    }");
                sb.AppendLine();
            }
        ...
        sb.AppendLine("            if (tokens.Count == 0)");
        sb.AppendLine("            {");
        sb.AppendLine("                throw new InvalidOperationException($\"没有找到匹配 Channel '{channelName}' 的事件监听器接口。请确保提供的处理器实现了相应的接口。\");");
        sb.AppendLine("            }");
        sb.AppendLine();
        ...
        // 生成便捷的重载方法（params 参数）
        sb.AppendLine("        /// <summary>");
        sb.AppendLine("        /// 自动注册所有匹配指定 Channel 的事件监听器（便捷重载）");
        sb.AppendLine("        /// </summary>");
        sb.AppendLine("        public static Task<IList<ISubscriptionToken>> AutoRegisterEventListenersAsync(");
        sb.AppendLine("            this IPulseClient client,");
        sb.AppendLine("            string channelName,");
        sb.AppendLine("            params object[] handlers)");
        sb.AppendLine("        {");
        sb.AppendLine("            return AutoRegisterEventListenersAsync(client, channelName, handlers, null, default);");
        sb.AppendLine("        }");
        sb.AppendLine();
    ...
    /// 从事件类型获取 Channel 名称
    /// </summary>
    private static string GetChannelName(INamedTypeSymbol eventType)
    {
        // 检查 ChannelAttribute
        var channelAttr = eventType.GetAttributes()
        ...
        if (channelAttr?.ConstructorArguments.Length > 0)
        {
            ...
            {
                return channelName!; // Null-forgiving operator: IsNullOrWhiteSpace guarantees non-null
            }
        }

        // 检查是否继承 IPulseReceiver（默认为 CLIENT）
        if (eventType.AllInterfaces.Any(i => i.Name == "IPulseReceiver"))
        {
            return "CLIENT";
        }
