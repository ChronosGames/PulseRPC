...
public static class EventHandlerGenerator
{
    ...
    public static string GenerateEventHandler(INamedTypeSymbol interfaceSymbol, SourceProductionContext context, Dictionary<string, ushort> protocolIds)
    {
        ...
        // 生成事件处理器类（只生成特定于此接口的处理器）
        GenerateHandlerClass(sb, interfaceSymbol, handlerClassName, channelName, protocolIds, isGlobalNamespace);
    ...
    private static void GenerateFileHeader(StringBuilder sb)
    {
        ...
        sb.AppendLine("// PulseRPC Event Handler Generator");
    ...
    private static void GenerateHandlerClass(StringBuilder sb, INamedTypeSymbol interfaceSymbol, string handlerClassName, string channelName, Dictionary<string, ushort> protocolIds, bool isGlobalNamespace)
    {
        var interfaceName = interfaceSymbol.Name;
        ...
        sb.AppendLine($"{indent}/// 事件处理器 - 为 {interfaceName} 提供高性能事件处理功能");
        sb.AppendLine($"{indent}/// 使用协议号进行高性能事件路由");
        sb.AppendLine($"{indent}/// </summary>");
        var eventHandlerClassName = interfaceSymbol.Name.StartsWith("I")
            ? $"{interfaceSymbol.Name.Substring(1)}EventHandler"
            : $"{interfaceSymbol.Name}EventHandler";
        sb.AppendLine($"{indent}public sealed class {eventHandlerClassName} : IDisposable");
        ...
        sb.AppendLine($"{memberIndent}public {eventHandlerClassName}()");
        sb.AppendLine($"{memberIndent}{{");
        sb.AppendLine($"{bodyIndent}_batchProcessor = new BatchProcessor(_metrics);");
        sb.AppendLine($"{memberIndent}}}");
        sb.AppendLine();

        sb.AppendLine($"{memberIndent}internal {eventHandlerClassName}(EventMetrics? customMetrics)");
        sb.AppendLine($"{memberIndent}{{");
    ...
    private static void GenerateSubscriptionMethods(StringBuilder sb, INamedTypeSymbol interfaceSymbol, string memberIndent, string bodyIndent)
    {
        ...
        sb.AppendLine($"{memberIndent}/// </summary>");
        sb.AppendLine($"{memberIndent}public void Subscribe({interfaceName} receiver, SubscriptionOptions? options = null)");
        sb.AppendLine($"{memberIndent}{{");
        sb.AppendLine($"{bodyIndent}ThrowIfDisposed();");
        sb.AppendLine($"{bodyIndent}options ??= SubscriptionOptions.Default;");
        sb.AppendLine();
        ...
        sb.AppendLine($"{memberIndent}/// </summary>");
        sb.AppendLine($"{memberIndent}public Task<ISubscriptionToken> SubscribeAsync({interfaceName} receiver, SubscriptionOptions? options = null)");
        sb.AppendLine($"{memberIndent}{{");
        ...
        sb.AppendLine($"{bodyIndent}var token = new SubscriptionToken(null!, context!, () => Unsubscribe(receiver));");
        ...
        foreach (var member in interfaceSymbol.GetMembers())
        {
            ...
            {
                GenerateDispatchMethod(sb, methodSymbol, interfaceName, memberIndent, bodyIndent);
            }
        }
    }

    private static void GenerateDispatchMethod(StringBuilder sb, IMethodSymbol methodSymbol, string interfaceName, string memberIndent, string bodyIndent)
    {
        ...
        sb.AppendLine($"{memberIndent}/// <summary>");
        sb.AppendLine($"{memberIndent}/// 分发 {methodName} 事件");
        sb.AppendLine($"{memberIndent}/// </summary>");
