...
PulseRPC 客户端源代码生成器，为 `IPulseHub` 接口（含标注 `[Channel("CLIENT")]` 的推送接收器）自动生成高性能代理和调度代码。
...
| 接口类型 | 生成文件 | 说明 |
|----------|----------|------|
| `IPulseHub` | `{Namespace}_{TypeName}.Stub.g.cs` | 服务 Stub 类，封装 RPC 调用 |
| `[Channel("CLIENT")] : IPulseHub` | `{Namespace}_{TypeName}.Dispatcher.g.cs` | 接收器调度器，处理事件反序列化和分发 |
| `[Channel("CLIENT")] : IPulseHub` | `{Namespace}_{TypeName}.EventHandler.g.cs` | 事件处理器（可选，支持批处理、监控等高级功能） |

> **命名规则**：接口名称会自动去掉 `I` 前缀。例如 `IPlayerHub` 生成 `PlayerHub.Stub.g.cs`
...
```xml
<PropertyGroup>
  <!-- 是否生成 EventHandler（默认 false） -->
  <PulseRPC_GenerateEventHandlers>true</PulseRPC_GenerateEventHandlers>
...
### EventHandler vs Dispatcher

| 特性 | Dispatcher | EventHandler |
|------|------------|--------------|
| 反序列化 | ✓ | ✓ |
| 事件分发 | ✓ | ✓ |
| 批量处理 | ✗ | ✓ |
...
**建议**：默认使用 `Dispatcher`（轻量），仅在需要高级功能时启用 `EventHandler`。
...
// Receiver 接口（客户端实现，标注 [Channel("CLIENT")] 表示"客户端实现、服务端推送"）
[Channel("CLIENT")]
...
// 或使用统一注册（注册对象实现的所有 [Channel("CLIENT")] : IPulseHub 推送接收器接口）
...
当存在 `[Channel("CLIENT")] : IPulseHub` 推送接收器接口时，会生成以下支持类型：

- `SubscriptionOptions` - 订阅配置（Default/Game/Critical 预设）
