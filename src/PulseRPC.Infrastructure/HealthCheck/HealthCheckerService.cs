...
public class HealthCheckerService : BackgroundService
{
        ...
        IOptions<HealthCheckOptions> healthCheckOptions)
    {
        ...
        _options = healthCheckOptions?.Value ?? throw new ArgumentNullException(nameof(healthCheckOptions));
    }
    ...
    private async Task PerformHealthCheckForServiceAsync(string serviceName, CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            var healthResults = await healthChecker.CheckHealthBatchAsync(endpoints.ToList(), cancellationToken);
    ...
    private async Task ProcessHealthChangesAsync(
        IReadOnlyDictionary<string, HealthCheckResult> healthResults)
    {
        ...
        foreach (var kvp in healthResults)
        {
            ...
            var endpoint = result.ToEndpoint();
            var newStatus = result.Status;

            var oldStatus = _lastHealthStates.GetValueOrDefault(id, HealthStatus.Unknown);

            // 更新健康状态
            _lastHealthStates[id] = result.Status;
